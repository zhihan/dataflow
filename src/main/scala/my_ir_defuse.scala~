...
object BooleanMapSet {
  ...
  def usedSet(g: Graph, s:Map[Vertex,Statement], variable: Var) = {
    ...
    def addIf = (m: Map[Vertex,Boolean], v : Vertex) => 
      m + (if (isUsed(variable, s(v)))  v -> true else  v-> false )

    val m = g.V.foldLeft (Map[Vertex,Boolean]()) (addIf)
    new BooleanMapSet(m)
  }

  def modifiedSet(g: Graph, s:Map[Vertex,Statement], variable: Var) = {
    ...
    def addIf = (m: Map[Vertex,Boolean], v : Vertex) => 
      m + (if (isModified(variable, s(v))) { v -> true } else { v-> false} )
